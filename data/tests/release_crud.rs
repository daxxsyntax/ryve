...
async fn create_roundtrips_and_parses_acceptance(pool: sqlx::SqlitePool) {
    let rel = release_repo::create(&pool, new_release("1.2.3")).await.unwrap();
...
async fn add_and_list_member_epics(pool: sqlx::SqlitePool) {
    let rel = release_repo::create(&pool, new_release("0.1.0")).await.unwrap();

    release_repo::add_epic(&pool, &rel.id, "sp-0001").await.unwrap();
    release_repo::add_epic(&pool, &rel.id, "sp-0003").await.unwrap();

    let members = release_repo::list_member_epics(&pool, &rel.id).await.unwrap();
    assert_eq!(members, vec!["sp-0001".to_string(), "sp-0003".to_string()]);

    release_repo::remove_epic(&pool, &rel.id, "sp-0001").await.unwrap();
    let members = release_repo::list_member_epics(&pool, &rel.id).await.unwrap();
...
async fn epic_rejected_when_in_another_open_release(pool: sqlx::SqlitePool) {
    let a = release_repo::create(&pool, new_release("1.0.0")).await.unwrap();
    let b = release_repo::create(&pool, new_release("1.1.0")).await.unwrap();

    release_repo::add_epic(&pool, &a.id, "sp-0002").await.unwrap();
...
async fn epic_allowed_once_other_release_is_cut(pool: sqlx::SqlitePool) {
    let a = release_repo::create(&pool, new_release("2.0.0")).await.unwrap();
    let b = release_repo::create(&pool, new_release("2.1.0")).await.unwrap();

    release_repo::add_epic(&pool, &a.id, "sp-0004").await.unwrap();
    ...
    release_repo::add_epic(&pool, &b.id, "sp-0004").await.unwrap();
}
...
async fn set_status_reopen_conflict_is_typed(pool: sqlx::SqlitePool) {
    ...
    release_repo::add_epic(&pool, &a.id, "sp-0001").await.unwrap();
    release_repo::set_status(&pool, &a.id, ReleaseStatus::Cut).await.unwrap();

    // sp-0001 now lives in B as the sole open release.
    release_repo::add_epic(&pool, &b.id, "sp-0001").await.unwrap();
...
async fn list_filters_by_status(pool: sqlx::SqlitePool) {
    let a = release_repo::create(&pool, new_release("4.0.0")).await.unwrap();
    let _b = release_repo::create(&pool, new_release("4.1.0")).await.unwrap();
