...
pub async fn create(pool: &SqlitePool, session: &NewAgentSession) -> Result<(), SparksError> {
    ...
    sqlx::query(
        "INSERT INTO agent_sessions (id, workshop_id, agent_name, agent_command, agent_args, session_label, status, started_at, child_pid, resume_id, log_path, parent_session_id, archetype_id)
         VALUES (?, ?, ?, ?, ?, ?, 'active', ?, ?, ?, ?, ?, ?)",
    )
    .bind(&session.id)
