...
use iced::widget::{
    Column, Space, button, container, mouse_area, row, scrollable, text, text_input,
};
...
pub fn view<'a>(state: &'a FileViewerState, pal: &Palette, has_bg: bool) -> Element<'a, Message> {
    ...
    let viewer: Element<'a, Message> = scrollable(code_column)
    ...
    if state.search_open {
        Column::new()
            .push(view_search_bar(state, pal))
            .push(viewer)
            .width(Length::Fill)
            .height(Length::Fill)
            .into()
